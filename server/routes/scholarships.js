...
router.get("/scraping-stats", auth, adminAuth, async (req, res) => {
  try {
    ...
    // Get the most recent successful scraping session
    const lastSuccessfulScrape = await ScrapingSession.findOne({
      status: "success",
    })
      .sort({ createdAt: -1 })
      .select("createdAt endTime")
      .lean();

    res.json({
      success: true,
      data: latestSession,
      lastSuccessfulScrape: lastSuccessfulScrape
        ? lastSuccessfulScrape.createdAt
        : null,
    });
  } catch (error) {
...
router.post("/cancel-scraping", auth, adminAuth, (req, res) => {
  if (scrapingState.isRunning) {
    ...
    res.json({
      success: true,
      message:
        "Cancellation requested. Scraping will stop after current scholarship.",
    });
  } else {
...
router.get("/", async (req, res) => {
  try {
    ...
    // If latestSessionOnly=true, filter by most recent scrapingSessionId
    if (latestSessionOnly === "true" || latestSessionOnly === true) {
      const latestSession = await require("../models/ScrapingSession")
        .findOne({
          status: { $in: ["success"] },
        })
        .sort({ createdAt: -1 });
      if (latestSession && latestSession._id) {
        query.scrapingSessionId = latestSession._id;

        // Check if any scholarships exist with this session
        const count = await Scholarship.countDocuments(query);
        if (count === 0) {
          console.log(
            "⚠️ No scholarships found for latest session. Returning empty list."
          );
        }
      } else {
        // No session found, but don't return empty - return all scholarships
        console.log(
          "⚠️ No successful scraping session found, returning all scholarships"
        );
      }
    }
...
router.get("/scrape-scholarships", async (req, res) => {
  try {
    const clearDatabase =
      String(req.query.clearDatabase || "false").toLowerCase() === "true";
        ...
        const tryParseTextual = (text) => {
          ...
          if (m1) {
            ...
            const mm = months.findIndex(
              (m) => m.toLowerCase() === m1[2].toLowerCase()
            );
          ...
          if (m2) {
            const mm = months.findIndex(
              (m) => m.toLowerCase() === m2[1].toLowerCase()
            );
      ...
      try {
        ...
        if (scrapingState.shouldCancel) {
          ...
          const duration = Math.floor(
            (endTime - scrapingState.startTime) / 1000
          );
          ...
          const successCount =
            scrapingState.savedCount + scrapingState.updatedCount;
        ...
        // Create ScrapingSession first
        const endTime = new Date();
        const duration = Math.floor((endTime - scrapingState.startTime) / 1000);
        const totalProcessed = scrapedScholarships.length;
        const successCount = scrapedScholarships.length; // Will update after saving
        let status = "success";
        let sessionErrors = [];

        let scrapingSession = null;
        try {
          scrapingSession = await ScrapingSession.create({
            startTime: scrapingState.startTime,
            endTime,
            duration,
            totalProcessed,
          ...
          console.error("❌ Error creating scraping session:", sessionError);
          sessionErrors.push(sessionError.message);
        }

        // Now save scholarships with session ID
        scrapingState.savedCount = 0;
        scrapingState.updatedCount = 0;
        scrapingState.failedCount = 0;
        for (let i = 0; i < scrapedScholarships.length; i++) {
          ...
          try {
            ...
            normalized.scrapingSessionId = scrapingSession
              ? scrapingSession._id
              : null;
            ...
            scrapingState.failedCount++;
            const errorMsg = `Failed to save ${scholarshipData.title}: ${error.message}`;
            scrapingState.errors.push(errorMsg);
          }
        }

        // Update scraping session with final stats
        ...
        // Update the existing session instead of creating a duplicate
        if (scrapingSession && scrapingSession._id) {
          try {
            await ScrapingSession.findByIdAndUpdate(scrapingSession._id, {
              totalProcessed:
                scrapingState.savedCount +
                scrapingState.updatedCount +
                scrapingState.failedCount,
        ...
        // Save failed session
        const endTime = new Date();
        const duration = scrapingState.startTime
          ? Math.floor((endTime - scrapingState.startTime) / 1000)
          : 0;
        const totalProcessed =
          scrapingState.savedCount +
          scrapingState.updatedCount +
...
router.get("/matches/:userId", auth, async (req, res) => {
  try {
    ...
    if (req.params.userId === "admin-1") {
      return res.status(400).json({
        message:
          "Admin account cannot use scholarship matching. Please create a student account.",
      });
    }
    ...
    const studentProgram = normalize(
      user?.profile?.program || user?.profile?.major || ""
    );
    ...
    for (const scholarship of scholarships) {
      ...
      if (!deadlineOk) {
        console.log(
          `⏰ Skipping expired scholarship: ${scholarship.title} (deadline: ${scholarship.deadline})`
        );
        expiredCount++;
      ...
      const cgpaOk =
        minGpa === null || isNaN(minGpa) ? true : studentCgpa >= minGpa;

      // 3. Check program/course requirements
      const courses = Array.isArray(scholarship.eligibleCourses)
      ...
      const isEducationLevelOnly =
        validCourses.length > 0 &&
        validCourses.every((c) => {
          const cc = normalize(c);
          return (
            cc === "diploma" ||
            cc === "degree" ||
      ...
      const programOk =
        !hasCourseConstraint || isOpenToAll || isEducationLevelOnly
          ? true
          : validCourses.some((c) => {
              const cc = normalize(c);
              ...
              for (const coursePart of courseParts) {
                ...
                if (programField && courseField) {
                  ...
                  if (
                    ...
                    courseField.split(" ").length >= 2
                  )
                    return true;
                  if (
                    courseField.includes(programField) &&
                    programField.split(" ").length >= 2
                  )
                    return true;
                }
              }
      ...
      if (deadlineOk && cgpaOk && programOk) {
        ...
        if (!cgpaOk)
          reasons.push(
            `CGPA too low (requires ${minGpa}, you have ${studentCgpa})`
          );
        if (!programOk) {
          if (validCourses.length > 0) {
            reasons.push(
              `Program doesn't match (requires: ${validCourses
                .slice(0, 3)
                .join(", ")}${validCourses.length > 3 ? "..." : ""})`
            );
          } else {
        ...
        console.log(
          `❌ Filtered out: ${scholarship.title} (Reasons: ${reasons.join(
            "; "
          )})`
        );
      }
    ...
    console.log(
      `📊 Filtering results: ${matches.length} matches, ${nonMatches.length} non-matches, ${expiredCount} expired`
    );
...
router.get("/debug/filtering", async (req, res) => {
  try {
    const now = new Date();
    // Allow ?all=true to show all scholarships, not just active
    const showAll = req.query.all === "true";
    const scholarships = showAll
      ? await Scholarship.find({})
      : await Scholarship.find({ status: "active" });

    // Get latest successful scraping session
    const latestSession = await require("../models/ScrapingSession")
      .findOne({
        status: { $in: ["success"] },
      })
      .sort({ createdAt: -1 });
    const latestSessionId = latestSession ? String(latestSession._id) : null;

    const withLatestSession = scholarships.filter(
      (s) =>
        s.scrapingSessionId && String(s.scrapingSessionId) === latestSessionId
    );
    ...
    const debugInfo = {
      ...
      scholarships: scholarships.map((scholarship) => ({
        ...
        scrapingSessionId: scholarship.scrapingSessionId,
        isLatestSession:
          scholarship.scrapingSessionId &&
          String(scholarship.scrapingSessionId) === latestSessionId,
        createdAt: scholarship.createdAt,
        updatedAt: scholarship.updatedAt,
      })),
      countWithLatestSession: withLatestSession.length,
      idsWithLatestSession: withLatestSession.map((s) => s._id),
    };

    console.log("[DEBUG] Latest session ID:", latestSessionId);
    console.log(
      "[DEBUG] Scholarships with latest session:",
      withLatestSession.length
    );
    res.json(debugInfo);
  } catch (error) {
...
router.post("/public-matches", async (req, res) => {
  try {
    ...
    for (const scholarship of scholarships) {
      ...
      if (!deadlineOk) {
        console.log(
          `⏰ Public: Skipping expired scholarship: ${scholarship.title} (deadline: ${scholarship.deadline})`
        );
        expiredCount++;
        continue; // Skip expired scholarships
      }
    ...
    console.log(
      `📊 Public filtering results: ${matches.length} matches, ${nonMatches.length} non-matches, ${expiredCount} expired`
    );
    res.json({ matches, nonMatches });
  } catch (error) {
